...
class CriarReservaTest(TestCase):
    def setUp(self):
        ...
        self.anuncio = Anuncio.objects.create(pontos=10, img1='', img2='', descricao='Descricao Teste', veiculo=self.veiculo, destaque=False, preco=20000, servico=False)
        self.reserva_data = {
            'email': 'user@test.com',
            'id': self.anuncio.id
        }
    ...
    def test_criar_reserva(self):
        ...
        response = self.client.post(reverse('Reservar'), self.reserva_data, format='json')
